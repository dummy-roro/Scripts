pipeline {
    ...
    stage('Checkout from Git'){
            steps{git
                git branch: 'main', url: 'your git repository url here'
            }
        }    
                    ...
                    withSonarQubeEnv('sonar-qube-server') {
                        ...
                        sh '''
                            $SONAR_SCANNER_HOME/bin/sonar-scanner \
                                -Dsonar.projectKey=<yoour projectKey> \
        ...
        stage('Trivy Vulnerability Scanner') {
            steps {
                sh  ''' 
                    trivy image <dockerhub repo>/<image-name>:$GIT_COMMIT \
        ...
        stage('Push Docker Image') {
            steps {
                withDockerRegistry(credentialsId: 'docker-hub-credentials', url: "") {
                    sh  'docker push <dockerhub repo>/<image-name>:$GIT_COMMIT'
                }
            }
        }
        ...
        stage('K8S - Update Image Tag') {
            ...
            steps {
                sh 'git clone -b main <your gitops repository url here>'
                dir("solar-system-gitops-argocd/kubernetes") {
                    sh '''
                        ...
                        git remote set-url origin <your gitops repository url here>
                        git add .
